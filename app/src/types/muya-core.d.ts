 ...
 * (vendored at app/packages/muya), scoped to what Notademics consumes.
 *
 ...
 * ships no built `lib/types/*.d.ts`. If tsc resolved the import to that source
 * it would type-check the entire muya tree under the app's program — producing
 * spurious errors (e.g. `?inline` font imports, `Intl.Segmenter` lib level,
 * `window.DIRNAME` globals). A `paths` entry in tsconfig.json redirects
 * `@muyajs/core` here, cutting the dependency graph at the import boundary
 * (the same shielding marktext's desktop gives via `muya-core.d.ts`).
 * Vite still resolves the real runtime module via the package `exports` map at
 * build time.
 */
...
declare module '@muyajs/core' {
  ...
  // that crosses the editor boundary was already `any` in the desktop shell.
  export class Muya {
